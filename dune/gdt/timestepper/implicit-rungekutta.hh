 ...
 *
 * Timestepper using diagonally implicit Runge Kutta methods to solve equations of the form u_t = r * L(u, t) where u is a
 ...
 * not contained in DiagonallyImplicitRungeKuttaMethods, choose TimeStepperMethods::diagonally_implicit_other and supply a
 ...
 * \mathbf{res}(\mathbf{u}_l + \alpha * \mathbf{d}_l) < \mathbf{res}(\mathbf{u}^n) + \beta D\mathbf{res} \alpha \mathbf{d}_l,
 ...
 */
template <class OperatorImp, class DiscreteFunctionImp, class TimeFieldImp = double, TimeStepperMethods method = TimeStepperMethods::trapezoidal_rule, XT::LA::Backends container_backend = XT::LA::default_sparse_backend>
...
public:
   ...
   */
  DiagonallyImplicitRungeKuttaTimeStepper(const OperatorType& op, const DiscreteFunctionType& initial_values,
    ...
    , num_stages_(A_.rows())
  {
    ...
    if (!diagonally_implicit)
      DUNE_THROW(Dune::NotImplemented,
                 "Matrix A has entries above the diagonal, this is not supported yet!" );
