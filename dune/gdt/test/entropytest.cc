...
auto function_factor(const DiscreteFunctionType& discrete_function) ->
...
struct static_for_loop
{
  ...
  static void sum_vector(const DiscreteFunctionType& discrete_function, FactorDiscreteFunctionType& first_discrete_function)
  {
   std::cout << "current factor " << current_factor_index << std::endl;
   first_discrete_function.vector() += function_factor<current_factor_index, DiscreteFunctionType>(discrete_function).vector();
   static_for_loop<current_factor_index + 1, last_factor_index>::sum_vector(discrete_function, first_discrete_function);
  }
};
...
struct static_for_loop<last_factor_index, last_factor_index>
{
  ...
  static void sum_vector(const DiscreteFunctionType& discrete_function, FactorDiscreteFunctionType& first_discrete_function)
  {
   first_discrete_function.vector() += function_factor<last_factor_index, DiscreteFunctionType>(discrete_function).vector();
  }
};
...
//  typedef typename Dune::GDT::Hyperbolic::Problems::SourceBeamPnHatFunctions< EntityType, double, dimDomain, double, momentOrder > ProblemType;
//  typedef typename Dune::GDT::Hyperbolic::Problems::SourceBeamPnLegendre< EntityType, double, dimDomain, double, momentOrder > ProblemType;
//  typedef typename Dune::GDT::Hyperbolic::Problems::PlaneSourcePnLegendre< EntityType, double, dimDomain, double, momentOrder > ProblemType;
  typedef typename Dune::GDT::Hyperbolic::Problems::PlaneSourcePnHatFunctions< EntityType, double, dimDomain, double, momentOrder > ProblemType;
//  const auto problem_ptr = ProblemType::create(ProblemType::default_config(true));
  ...
  const auto grid_ptr = Dune::XT::Grid::CubeGridProviderFactory<GridType>::create(ProblemType::default_grid_config()).grid_ptr();
  ...
  for (const auto& entity : elements(velocity_grid_view)) {
    const auto local_quadrature_rule = Dune::QuadratureRules<double, dimDomain>::rule(entity.type(),quadrature_order,Dune::QuadratureType::GaussLegendre);
    for (const auto& quad : local_quadrature_rule) {
      quadrature_rule.push_back(Dune::QuadraturePoint<double, dimDomain>(entity.geometry().global(quad.position()),
                                                                         quad.weight() * entity.geometry().integrationElement(quad.position())));
      std::cout << Dune::XT::Common::to_string(quadrature_rule.back().position()) << " and " << quadrature_rule.back().weight() << std::endl;
    }
  }
...
//      basis_values_matrix[ii][nn] = Dune::GDT::Hyperbolic::Problems::evaluate_legendre_polynomial(quadrature_rule[ii].position(), nn);
      basis_values_matrix[ii][nn] = Dune::GDT::Hyperbolic::Problems::evaluate_hat_function(quadrature_rule[ii].position()[0], nn, ProblemType::create_equidistant_points());
...
//      double, dimDomain, double, dimRange, 1, num_quad_points> AnalyticalFluxType;
  typedef typename Dune::GDT::EntropyBasedFluxHatFunctions<GridType, typename SpaceType::EntityType,
      double, dimDomain, double, dimRange, 1> AnalyticalFluxType;
  ...
  typedef typename Dune::GDT::TimeStepperFactory<AdvectionOperatorType, DiscreteFunctionType, RangeFieldType,
      time_stepper_method>::TimeStepperType OperatorTimeStepperType;
  typedef typename Dune::GDT::TimeStepperFactory<RHSOperatorType, DiscreteFunctionType, RangeFieldType,
...
//  const std::shared_ptr<const AnalyticalFluxType> analytical_flux = problem.flux();
  const auto analytical_flux = std::make_shared< const AnalyticalFluxType >(grid, ProblemType::create_equidistant_points());
...
//    timestepper.solve(t_end, dt, num_save_steps, false, true, visualize, "plane_source_legendre_p" + Dune::XT::Common::to_string(momentOrder) + "_implicit");
...
//      sum_function.visualize("hatfunctions_pn_implicit", Dune::XT::Common::to_string(counter));
      sum_function.visualize("plane_source_hatfunctions_p" + Dune::XT::Common::to_string(momentOrder) + "_implicit_entropy", Dune::XT::Common::to_string(counter));
      ++counter;
    }
