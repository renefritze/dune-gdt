...
class PlaneSourcePnLegendre : public SourceBeamPnLegendre<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, momentOrder>
{
...
public:
  ...
  PlaneSourcePnLegendre(const std::shared_ptr<const FluxType> flux_in, const std::shared_ptr<const RHSType> rhs_in,
...
protected:
  ...
  static void create_initial_value_config(ConfigType& initial_value_config, const ConfigType& grid_config = default_grid_config(), const RangeFieldImp psi_vac = 5e-9)
  {
...
class PlaneSourcePnHatFunctions : public SourceBeamPnHatFunctions<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, num_points>
{
...
public:
                                          ...
                                          const std::string sub_name = static_id())
  {
   ...
   static ConfigType default_config(const RangeType& v_points = create_equidistant_points(), const RangeFieldImp psi_vac = 5e-9)
  {
  ...
  PlaneSourcePnHatFunctions(const std::shared_ptr<const FluxType> flux_in, const std::shared_ptr<const RHSType> rhs_in,
             const std::shared_ptr<const InitialValueType> initial_values_in, const ConfigType& grid_config_in,
             const ConfigType& boundary_info_in, const std::shared_ptr<const BoundaryValueType> boundary_values_in)
    : BaseType(flux_in, rhs_in, initial_values_in, grid_config_in, boundary_info_in, boundary_values_in)
  {
...
protected:
  ...
  // Initial value of the kinetic equation is psi_vac + delta(x).
  // Thus the initial value for the n-th moment is base_integrated_n * (psi_vac + delta(x))
  static void create_initial_value_config(ConfigType& initial_value_config, const ConfigType& grid_config = default_grid_config(), const RangeFieldImp psi_vac = 5e-9, const RangeType& v_points = create_equidistant_points())
  {
