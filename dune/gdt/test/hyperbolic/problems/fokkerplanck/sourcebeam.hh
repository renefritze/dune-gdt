...
FieldType evaluate_hat_function(const FieldType& v, const size_t n, const RangeType& v_points)
{
  ...
  else if (n==v_points.size()
           && XT::Common::FloatCmp::ge(v, v_points[v_points.size()-2])
...
public:
  ...
  static ConfigType default_config(const bool BGK_type_collision = false, const RangeFieldImp psi_vac = 5e-9)
  {
    ...
    boundary_value_config["expression"] = BGK_type_collision ? create_realizable_boundary_values(psi_vac)
  ...
  SourceBeamPnLegendre(const std::shared_ptr<const FluxType> flux_in, const std::shared_ptr<const RHSType> rhs_in,
...
template <class EntityImp, class DomainFieldImp, size_t domainDim, class RangeFieldImp, size_t momentOrder>
std::vector<RangeFieldImp> SourceBeamPnLegendre<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, momentOrder>::left_boundary_vals = {1.0, 0.99821587588384722888546103381424, 0.99465512765154168665638310109402, 0.98933271069998046949348833831956, 0.98227094694487696211966529188208, 0.973499392321142130859194712695, 0.96305466163489000577549441482268, 0.95098021205162953191200986872772, 0.93732608680563406367139508178687, 0.9221486210023942981265256212649, 0.9055101116570464980458926840956, 0.88747845436446410855581951816166, 0.86812674922921981743467893406176, 0.84753287889397251365597836272049, 0.82577906169132591028268357189113, 0.80295138310538515338857079818232, 0.77913930886388561354086750071819, 0.75443518308891443080916196222167, 0.72893371501317733202250980561682, 0.70273145781902986912513049892392, 0.67592628317890714138498882739631, 0.6486168550684271196351824164262, 0.62090210638764909412176999841986, 0.5928807218623355455816081929013, 0.56465063060643566894968702250824, 0.53630851161046188339453821024199, 0.50794931527927140373971781871463, 0.47966580397850727702579631268181, 0.4515481143633037274085705597037, 0.42368334405769961332718708268865, 0.39615516503056535291425143183468, 0.3690434657758989934336017941775};


/** \see class TwoBeams in twobeams.hh */
template <class EntityImp, class DomainFieldImp, size_t domainDim, class RangeFieldImp, size_t num_points>
class SourceBeamPnHatFunctions : public SourceBeamPnLegendre<EntityImp, DomainFieldImp, domainDim, RangeFieldImp, num_points-1>
{
...
public:
  ...
  SourceBeamPnHatFunctions(const std::shared_ptr<const FluxType> flux_in, const std::shared_ptr<const RHSType> rhs_in,
                           const std::shared_ptr<const InitialValueType> initial_values_in, const ConfigType& grid_config_in,
                           const ConfigType& boundary_info_in, const std::shared_ptr<const BoundaryValueType> boundary_values_in)
    : BaseType(flux_in, rhs_in, initial_values_in, grid_config_in, boundary_info_in, boundary_values_in)
  {
...
protected:
  ...
  static MatrixType mass_matrix_with_v(const RangeType& v_points)
  {
    ...
    for (size_t rr = 0; rr < dimRange; ++rr) {
      if (rr > 0 && rr < dimRange-1)
        ret[rr][rr] = (v_points[rr+1]*v_points[rr+1] + 2*v_points[rr+1]*v_points[rr] - 2*v_points[rr]*v_points[rr-1] - v_points[rr-1]*v_points[rr-1])/12.;
      if (rr > 0)
    ...
    ret[dimRange-1][dimRange-1] = (3*v_points[dimRange - 1]*v_points[dimRange - 1]
        - 2*v_points[dimRange-1]*v_points[dimRange-2]
        - v_points[dimRange-2]*v_points[dimRange-2])/12.;
    return ret;
  }
  ...
  static std::string create_initial_values(const RangeType& v_points = create_equidistant_points(), const RangeFieldImp psi_vac = 5e-9)
  {
  ...
  {
    static constexpr auto pi = std::acos(-1);
    return 1/200.*std::sqrt(pi/10)*(std::erf(100*std::sqrt(10)*(v_u-1))-std::erf(100*std::sqrt(10)*(v_l-1)));
  }
  ...
  // at x = 3.
  // Simulate with linear interpolating function.
  static std::string create_boundary_values(const RangeType& v_points = create_equidistant_points(), const RangeFieldImp psi_vac = 5e-9)
  {
    const RangeType integrated_basis = hatfunctions_integrated(v_points);
    ...
    for (size_t nn = 0; nn < dimRange; ++nn) {
      ...
      if (nn < dimRange - 1)
        left_boundary_values[nn] += 1./((vn - vnp)*denominator()) * ( (1 - vnp) * integral_1(vn, vnp) - 1./2e5 * (numerator(vnp) - numerator(vn)));
      if (nn > 0)
        left_boundary_values[nn] += 1./((vn - vnm)*denominator()) * ( (1 - vnm) * integral_1(vnm, vn) - 1./2e5 * (numerator(vn) - numerator(vnm)));
    }
    ...
    for (size_t nn = 0; nn < dimRange; ++nn) {
      ...
      str += XT::Common::to_string(left_boundary_values[nn], precision)
          + "-(" + XT::Common::to_string(left_boundary_values[nn]-psi_vac*integrated_basis[nn], precision) + ")*x[0]/3.0";
    }
    str += "]";
    return str;
  } // ... create_boundary_values()
...
class SourceBeamTestCase
    : public Dune::GDT::Test::NonStationaryTestCase<G, Problems::SourceBeamPnLegendre<typename G::template Codim<0>::Entity,
