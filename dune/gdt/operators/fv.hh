...
public:
  ...
  void apply(const SourceType& source, RangeType& range, const double /*time*/ = 0.0) const
  {
    ...
    LocalVolumeIntegralFunctional<LocalFvRhsIntegrand<RHSEvaluationType, SourceType>> local_functional(rhs_evaluation_, source);
    VectorFunctionalBase<typename RangeType::VectorType,
                         typename RangeType::SpaceType,
                         typename RangeType::SpaceType::GridViewType,
                         typename RangeType::DomainFieldType> functional_assembler(range.vector(), range.space());
