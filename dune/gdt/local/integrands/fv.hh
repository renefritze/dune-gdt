...
public:
  ...
  typedef std::tuple<size_t, std::unique_ptr<typename SourceType::LocalfunctionType>, typename SourceType::SpaceType::GridViewType::ctype> LocalfunctionTupleType;
}; // class LocalFvRhsIntegrandTraits
...
public:
  ...
  void evaluate(const LocalfunctionTupleType& local_functions_tuple,
                ...
                const Dune::FieldVector<DomainType, dimDomain>& x_local, Dune::DynamicVector<R>& ret) const
  {
...
{
  typedef LocalVolumeIntegrandInterface<internal::LocalFvRhsJacobianIntegrandTraits<RhsEvaluationImp, SourceType>, 2> BaseType;
...
public:
  ...
  void evaluate(const LocalfunctionTupleType& local_functions_tuple,
                ...
                const Dune::FieldVector<DomainFieldType, dimDomain>& x_local, Dune::DynamicMatrix<R>& ret) const
  {
    const auto& entity = test_base.entity();
    const auto u = std::get<1>(local_functions_tuple)->evaluate(x_local);
