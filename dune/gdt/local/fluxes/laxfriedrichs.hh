                     ...
                     const Dune::FieldVector<DomainFieldType, dimDomain - 1>& x_intersection) const
  {
    ...
    *max_derivative_ = dx / dt_;
    if (use_local_) {
      if (!is_linear_ || !(*max_derivative_calculated_)) {
        *max_derivative_        = 0;
        const auto jacobian_u_i = analytical_flux_.jacobian(u_i, intersection.inside(), intersection.geometryInInside().global(x_intersection));
        const auto jacobian_u_j = analytical_flux_.jacobian(u_j, intersection.outside(), intersection.geometryInOutside().global(x_intersection));
    ...
    ret += analytical_flux_.evaluate(u_j, intersection.outside(), intersection.geometryInOutside().global(x_intersection));
    if (n_ij < 0)
      ret *= n_ij;
